const dialogs = (state={}, action={}) => {
  ...
  switch(action.type) {
    ...
    case 'CREATE_DIALOG_SUCCESS':
      return {...state, list: state.list.concat(response), newRedirect: response.id, isLoading: false, error: null};
    case 'CREATE_DIALOG_FAILURE':
      return {...state, newRedirect: null, isLoading: false, error};

    case 'REMOVE_DIALOG_REQUEST':
      return {...state, list: state.list.filter(d => d.id !== action.body.dialogID), isLoading: true, error: null};
        ...
        list: state.list.map(d =>
            ...
            { ...d,
              messages: d.messages.map(m => m.id === action.messageID ? {
                ...m,
                viewedBy: !m.viewedBy.some(vb => vb === action.userID) ? m.viewedBy.concat(action.userID) : m.viewedBy
              } : m)
            } :
              d
        )};
